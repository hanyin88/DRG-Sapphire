...
# did not import accuracy reward
...
from open_r1.rewards_strict import accuracy_reward, format_reward
from open_r1.utils import get_tokenizer
from open_r1.utils.callbacks import get_callbacks
from open_r1.utils.wandb_logging import init_wandb_training
from trl import GRPOTrainer, ModelConfig, ScriptArguments, TrlParser, get_peft_config

from open_r1.GRPO_Best_Trainer import GRPO_Best_Trainer
...
def main(script_args, training_args, model_args):
    ...
    # Ensure dataset is a Dataset, not a DatasetDict
    if isinstance(dataset, DatasetDict):  # Correct way to check
        dataset = dataset["train"]  # Extract the 'train' split
    ...
    # append the drg_intro to the start of prompt
    dataset = dataset.map(lambda x: {"problem": f"{drg_intro}{x['problem']}"})
    
    # make the dataset a datasetdict object and everything is train spilt
    dataset = datasets.DatasetDict({"train": dataset})   
    
    # from datasets import DatasetDict
    
    # # Only use 20% of the dataset for faster training

    # dataset =  DatasetDict({split: ds.shuffle(seed=42).select(range(int(len(ds) * 0.2)))
