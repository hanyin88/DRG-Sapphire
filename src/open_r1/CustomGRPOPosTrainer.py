...
class CustomGRPOPosTrainer(GRPOTrainer):
    """
    ...
    def _prepare_inputs(self, inputs: dict[str, Any]) -> dict[str, Any]:
        ...
        if self.accelerator.is_main_process:
            ...
            for i in range(num_prompt_groups):
                ...
                attempts = 0
                while (std_reward < self.regen_min_std or not (rewards > self.min_award).any().item()) and attempts < self.regen_max_attempts:

        # Print whether the min_award was met or not, and print out rewards. Note rewards is a tensor
                    if not (rewards > self.min_award).any().item():
                        print(f"Regeneration needed due to low reward. Rewards: {rewards.tolist()}, Attempts: {attempts}")
                    # else:
                    #     print(f"Regeneration not needed. Rewards: {rewards.tolist()}, Attempts: {attempts}")
